...
def ventas_list(request):
    ...
    try:
        with transaction.atomic():
            ...
            if p.stock < cantidad:
                return Response({'error': f'Stock insuficiente. Disponible: {p.stock}'}, status=400)
            p.stock -= cantidad
...
def ventas_multiple(request):
    ...
    try:
        with transaction.atomic():
            ventas_creadas = []
            grupo = str(uuid.uuid4())  # mismo grupo para todos los items
